---
layout: ../../layouts/telegram-post.astro
title: 🧑‍💻 Giga Code
publishDate: 2024-05-06T10:00:02.000Z
updatedDate: 2024-05-06T11:48:27.000Z
telegramId: 197
telegramMessageIds:
  - 197
  - 198
  - 199
  - 200
  - 201
telegramGroupedId: "13719916822872162"
channel: "@cherkashindev"
views: 1860
forwards: 8
reactions:
  - reaction: ❤
    count: 3
  - reaction: 👍
    count: 14
  - reaction: 🔥
    count: 2
  - reaction: 🤡
    count: 1
media:
  - /telegram-media/msg-197-1.jpg
  - /telegram-media/msg-198-1.jpg
  - /telegram-media/msg-199-1.jpg
  - /telegram-media/msg-200-1.jpg
  - /telegram-media/msg-201-1.jpg
archiveExcerpt: |-
  🧑‍💻 Giga Code

  Месяц назад я отключил Codeium и честно начал тестировал GigaCode (альтернативу Copilot от Сбера). Но похоже придётся переключаться обратно, GigaCode ещё сыроват…

  Он, конечно, полезен, и действительно иногда упрощает написание кода,...
archiveTags:
  - ai
archiveImage:
  src: /telegram-media/archive/msg-197-1-archive.webp
  width: 720
  height: 720
---

**🧑‍💻 Giga Code**

Месяц назад я отключил [Codeium](https://t.me/cherkashindev/153) и честно начал тестировал [GigaCode](https://gigacode.ru/#/instruction/961d9cc0-98c5-11ee-b9d1-0242ac120002) \(альтернативу Copilot от Сбера\). Но похоже придётся переключаться обратно, GigaCode ещё сыроват…

Он, конечно, полезен, и действительно иногда упрощает написание кода, да и вообще круто, что появляются такие отечественные продукты.

Но пока то и дело \(ну очень часто\) возникает ощущение, что GigaCode путается под ногами, и мешает писать код, а вовсе не помогает \(карточка 4\):

- Он то и дело вставляет ненужные закрывающие скобки и кавычки, которые VS Code уже вставил за меня.
- Другая надоедливая ситуация — он не может уследить, что я уже написал часть слова и предлагает вставить её ещё раз.

В общем, пока ждём улучшенной версии …

@cherkashindev

#ai
