---
layout: ../../layouts/telegram-post.astro
title: peerDependencies
publishDate: 2024-06-10T10:03:27.000Z
updatedDate: 2024-06-10T10:04:31.000Z
telegramId: 211
telegramMessageIds:
  - 211
telegramGroupedId: null
channel: "@cherkashindev"
views: 1766
forwards: 18
reactions:
  - reaction: 👍
    count: 20
  - reaction: 🔥
    count: 4
media:
  - /telegram-media/msg-211-1.jpg
archiveExcerpt: |-
  peerDependencies

  Недавно делал код ревью и заметил, что в pnpm-lock.yaml (альтернатива yarn.lock в pnpm) добавлена 17-я версия реакта, хотя на проекте мы используем 18-ю. Нам не нужно тянуть в проект 2 версии реакта — поэтому идём разбираться.

  Дело...
archiveTags:
  - react
  - pnpm
archiveImage:
  src: /telegram-media/archive/msg-211-1-archive.webp
  width: 720
  height: 417
---

**peerDependencies**

Недавно делал код ревью и заметил, что в `pnpm-lock.yaml` \(альтернатива `yarn.lock` в pnpm\) добавлена 17-я версия реакта, хотя на проекте мы используем 18-ю. Нам не нужно тянуть в проект 2 версии реакта — поэтому идём разбираться.

Дело в том, что в библиотеке `react-cheetah-grid`, которую мы используем для рендера длинных таблиц, реакт указан в секции `dependencies` вместо `peerDependencies`.  

**dependencies vs peerDependencies** 

- `dependencies`: пакеты, которые необходимы для работы вашего приложения и устанавливаются автоматически
- `peerDependencies`: пакеты, которые должны быть уже установлены в среде, где используется ваш пакет, чтобы избежать конфликтов версий

Чтобы быстро исправить проблему — можно переопределить версию реакта для определённого пакета в `pnpm`  с помощью метода [readPackage.](https://pnpm.io/pnpmfile#hooksreadpackagepkg-context-pkg--promisepkg) Также нужно откатить изменения в `pnpm-lock.yaml` и запустить `pnpm install` .

```js

// .pnpmfile.cjs

const package = require('./package.json');

function readPackage(pkg, context) {
  if (pkg.name === "react-cheetah-grid") {
    pkg.dependencies['react'] = package.dependencies['react'];
    pkg.dependencies['react-dom'] = package.dependencies['react-dom'];
  }
  
  return pkg;
}

module.exports = {
  hooks: {
    readPackage,
  },
};
```

Но затем лучше создать ишью в репозитории библиотеки или отправить [пул реквест.](https://github.com/future-architect/cheetah-grid/pull/420/files) 

#react #pnpm
