...
- это приведёт к ре-рендеру компонента, так как каждый раз мы постоянно возвращает новый массив по ссылке, а [по-умолчанию используется строгое сравнение](https://github.com/pmndrs/zustand?tab=readme-ov-file#selecting-multiple-state-slices) \(old === new\).
...
[Автор Zustand упоминает](https://github.com/pmndrs/zustand/discussions/387#discussioncomment-731593), что можно нарушить согласованность данных и просто положить вычисляемые данные в стейт \(но в этом случае нужно вручную следить за их актуальностью\). 

Также он отмечает, что для этих целей можно использовать метод `memoize` из его библиотеки [proxy-memoize](https://github.com/dai-shi/proxy-memoize) \(для redux есть [reselect](https://github.com/reduxjs/reselect)\). 
